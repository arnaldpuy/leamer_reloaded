...
Y.vector <- c("jobs", "unemployed", "reduce_income_diff", "old_age_care", "housing", "health", 
              "jobs_c", "unemp_c", "incdiff_c", "oldage_c", "housing_c", "health_c", 
              "latent6", "latent4")
...
library(hrbrthemes)
library(viridis)
p = plot_scatter(data = mat, N = N, Y = out, params = params) + 
  geom_violin(trim = FALSE, fill = "skyblue", color = "darkblue") + 
  scale_fill_viridis(discrete = TRUE, alpha=0.6, option="C") +
  theme_minimal() +
  theme(
    axis.text.x = element_text(size = 9, angle = 15, hjust = 1)  # aumentato e ruotato per leggibilità
  ) +
  labs(x = "", y = "AME") + 
  scale_x_discrete(guide = guide_axis(n.dodge = 2)) +
...
original.plot<- cowplot::ggdraw() + cowplot::draw_image("fig01.png", scale = 1.5)
...
pdf(file = "All.pdf", width = 14, height = 7)  # più largo del default
